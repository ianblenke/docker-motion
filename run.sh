...
#
# This file does not contain all possible configuration options.
...
; vid_control_params brightness=125
#
...
snapshot_filename %Y/%m/%d/%H/%M/%v-%Y%m%d%H%M%S-snapshot
...
picture_filename %Y/%m/%d/%H/%M/%v-%Y%m%d%H%M%S-%q

# File path for motion triggered ffmpeg films (movies) relative to target_dir
# Default: %v-%Y%m%d%H%M%S
# File extensions(.mpg .avi) are automatically added so do not include them
movie_filename %Y/%m/%d/%H/%M/%v-%Y%m%d%H%M%S

# File path for timelapse movies relative to target_dir
# Default: %Y%m%d-timelapse
# File extensions(.mpg .avi) are automatically added so do not include them
timelapse_filename %Y/%m/%d/%H/%M/%Y%m%d-timelapse
...
# Output for http server, 0=css format, 1=text
...
# Command to execute to move a generic PTZ camera (default: none)
